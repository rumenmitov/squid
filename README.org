...
Since this work has been inspired by the Squid Cache, the snapshot mechanism is referred to as Squid Snapshot. The root directory in which all snapshots are stored is =/<squid-root>=, where <squid-root> is the path to all the squid snapshots (by default this is =/squid-root=).
